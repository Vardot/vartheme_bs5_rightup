...
render and for `include('vartheme_bs5_rightup:<name>', …)` sub-components.
...
- **Mirror every change** to the products copy:
  `/var/www/html/products/vartheme_bs5_rightup/` (same sub-path).
...
6. Mirrored to `/var/www/html/products/vartheme_bs5_rightup/`.
...
`sdc.vartheme_bs5_rightup.button`). So forking/renaming the theme changes every
...
4. Add the preload link in `MYTHEME/templates/includes/preload.twig`:
   ```twig
   <link rel="preload" href="/{{ vartheme_bs5_rightup_path }}/fonts/<name>/<File>.ttf"
         as="font" type="font/ttf" crossorigin="anonymous">
   ```
   `preload.twig` is included from `html.html.twig`; available vars:
   `vartheme_bs5_rightup_path` (theme path) and `html_dir` (`ltr`/`rtl`).

## 21. Responsive breakpoints (Bootstrap 5)

`xs` 0 · `sm` 576 · `md` 768 · `lg` 992 · `xl` 1200 · `xxl` 1400 px
(`vartheme_bs5_rightup.breakpoints.yml`). Theme regions are defined in
`vartheme_bs5_rightup.info.yml` (Header, Content, Footer + more).
