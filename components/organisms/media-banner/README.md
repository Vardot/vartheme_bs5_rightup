...
```twig
{% embed 'vartheme_bs5_rightup:media-banner' with {
...
- The background image layer is only rendered when a `media` source is resolved; it is decorative (`aria-hidden="true"`) and rendered through `vartheme_bs5_rightup:image` with `object-fit-cover` and eager loading.
